...
def get_cache_schools_version(url: str):
    ...
    cache_version_key = cache.get(CONF.CACHE_SCHOOLS_VERSION_KEY.format(country_id)) or 0
...
def invalidate_cache(country_id: int):
    ...
    """
    key = CONF.CACHE_SCHOOLS_VERSION_KEY.format(str(country_id))
    try:
        cache.incr(key)
    except ValueError:
        cache.set(key, 1)
...
def get_cache_schools_key(request: Request):
    ...
    return 'schools-etag-%s-%s' % (get_cache_schools_version(slugify(url)), slugify(url))

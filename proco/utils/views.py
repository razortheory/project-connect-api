...
class TransitionUnavailableError(APIException):
    status_code = status.HTTP_423_LOCKED
    default_detail = _('Transition is unavailable.')
...
class FSMTransitionActionMixin(object):
    ...
    def check_transition_permission(self, transition, user):
        try:
            ...
            if not has_transition_perm(transition, user):
    ...
    def get_transition_serializer_class(self, transition):
        ...
        im_self = getattr(transition, 'im_self', getattr(transition, '__self__'))  # noqa: B009
